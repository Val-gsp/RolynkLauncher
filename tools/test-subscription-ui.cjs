...
const markup = ejs.render(shop, { lang: key => key.split('.').reduce((o, k) => o[k], strings.ejs) })
fs.writeFileSync(path.join(artifacts, 'preview.html'), `<html><head><meta charset="utf-8"><base href="file://${root}/app/"><link rel="stylesheet" href="assets/css/launcher.css"><style>body{background:radial-gradient(ellipse at top,#203b37,#0b1118);height:100vh}#shopContainer{top:0}</style></head><body>${markup}</body></html>`)
;(async () => {
